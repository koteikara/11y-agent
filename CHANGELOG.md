...
- 主な変更内容: `memory/project-state.md` の Decisions にあるレビュー体制の1行を書き替えた。設計とレビューは Opus 5.5、実装は別セッションの Opus が担当する。承認相当のレビュー、Codex の二次レビュー、ユーザーのマージ判断という流れは変えていない。2026-09-23 までの記録にある「Fable」は当時の設計・レビュー担当を指すものとして、書き替えずに残した。
...
## 2026-09-18: 構造変更1 S3（再導出と照合、排他グループ）

- 背景・目的: 候補の変換後HTMLが元のHTMLから固定で作られ、決定後に作り直されないために、依存する候補が古いままになる問題（`goal2-app/TONO_FEEDBACK_FIX_INSTRUCTIONS.md` 2章の問題1・2）を、決定のたびの再導出で解消する。調停ロジックによる後追いをやめ、排他グループの宣言に置き換える。
- 主な変更内容:
  - 決定の一かたまりごとに1回、リプレイ→候補の作り直し→照合を走らせる（`rederiveCandidates()`）。AIの補完は呼ばない。
  - `reconcile()` で指紋により前の候補と突き合わせ、`candidate_id`・決定・AIの補完の結果を引き継ぐ。`patch`・`after_html`・`before_html`・`target.snippet` は作業中HTMLから作り直した値を使う。対象が無くなった未処理の候補は `withdrawn` としてログへ積む。
  - `EXCLUSIVE_GROUPS` を導入し、調停ロジック5関数（`resolveSupersededTableCandidates`・`resolveAlternativeMethodCandidates`・`survivesInAncestorOutput`・`isDescendantOfCandidateTarget`・`isTableRelatedCandidate`）を削除した。`conflicted` を新しく作る経路は無い。
  - リプレイの当て順を `seq` 順にし、決定を世代ごとにまとめるようにした。`order` はログから落とした。
  - 一括採用とGOAL1では「固定の変換後HTMLで差し替わる範囲の中にある候補は、その世代では採用しない」を守る。sg04015 で「令和５年度」の半角化が消えていた原因への対応。
...
- 背景・目的: 表の表示崩れを修正し、初期資料の課題候補を現在の実装・設計・検証記録と区別する。
- 主な変更内容: 表区切り3行を追加。B02・B03・B05・C02・C04・D01〜D04と本文抽出の根拠を更新。PR #132マージ後のS2実装済み状態・S3への持越しと、5自治体検証の未実施分を明記。D03・D04の旧記述は原本に存在することを照合したうえで現在の残課題へ更新。参照を相対リンクに変更。
- 関連ファイル: `memory/project-issue-candidates.md`、`CHANGELOG.md`
- 関連PR/コミット: PR #134（PR #133のマージ後レビューへの対応）。PR #132（マージ `dd62e30`）を反映。

## 2026-09-17: 移行・アクセシビリティ改善の課題候補を文書化

- 背景・目的: 初期資料から洗い出した課題候補を、優先度・根拠・影響・対応案・解消条件とともにプロジェクトへ統合する。
...
- 関連PR/コミット: PR #133（`62c247f`、マージ `8790c64`）。関連する理解サマリーはPR #29、取り込み時mainは `11acac4`。
...
- S1で決めた段階差（設計書 3.3・3.7・3.13 に追記）:
  ...
  - 当て順は、3.7の2が書く「`seq` 順に走査し、世代の境目で区切って当てる」ではなく、候補配列の添字が決める。ログが決めるのは「どの決定を当てるか」だけで、これで旧実装と構成上同じ順序になる。S1の候補の `after_html` は元のHTMLから固定で作られているため、`seq` 順に当てると出力が作業者の採用順で変わる（S1前は変わらなかった）。`seq` が当て順になるのは、候補が作業中HTMLから `after_html` を作り直すS2以降。
  - `orphaned` はS1では「修正が失われた」ではなく「リプレイで対象が見つからなかった」を意味する。畳み込みで祖先の `after_html` に入った修正も、対象の要素は祖先ごと差し替えられて消えるため印が立つが、修正自体は出力に残る。S3で画面に出すときに「失われた」と表示すると誤りになるので、`replay()` のコメントと設計書 3.7 に書いた。畳み込みが無くなるS2で本来の意味になる。
...
- 設計書との差異（設計書側も同じPRで直した）: 2章の行番号を現在の値に直し、`rebuildWorkingHtmlFor()` の当て順が（組の順序も組の中も）候補配列の並び順で決まり、決定した順を見ないこと、そのおかげで作業者の採用順に依存しないことを事実として書き足した。設計書はこの順序に触れていなかったが、S1のリプレイの当て順を決めるうえで欠かせない事実である。候補配列の並び順は「種類ごと、その中は文書順」で、コレクターの呼び出し順が先にくるため配列全体としては文書順ではない（表の中の見出しは表より前に並ぶ）ことも明記した。あわせて、決定済みの候補をもう一度決められることも書き足した。
- レビュー（Fable）で受けた要修正の対応: 当て順を `seq` 順から候補配列の添字順へ直した。`seq` 順だと画面で作業者が採用した順に出力が変わる。佐賀市の実ページ51件で、`autoAcceptSafe()` と同じ決定の集合を逆順に積むと3件（`sg02562`、`sg02571`、`sg03996`）、全候補を採用した集合では13件で出力が変わることを再現し、直したあとは逆順・無作為ともすべて一致することを確認した。原因は、同じ要素に「要素を残すパッチ」が2件以上あり、片方が元のHTML由来の値で要素の中身を書き戻す組（例: `file.file-display-text` の `set-text` と `text.alphanumeric` の「１」→「1」が同じ `<a>` に出る）。あわせて、`node_id` の文字列比較で組を並べていた点も直した（`assignNodeIds()` は4桁ゼロ埋めなので10000個目から `n10000` になり、`n10003` が `n9999` より前に並んで「外側を先」が崩れる）。候補配列の添字順にしたことで、この並べ替え自体が無くなった。
- 同値テスト（新規17件）: `replay()` と `rebuildWorkingHtmlFor()` の出力が一致することを、既存の `test/goal2-output` の入力（背景色の表、入れ子の表、複数修正の段落、複数修正を含む表、廃止属性混じりの本文、1列目がthの表、h3×4、セル結合のある表）に加え、レビューで挙がった「同じリンクへの文言の書き戻しと半角化」に対して確かめた。決定の集合は、GOAL1と同じ一括自動採用と、画面で作業者が進めた決定（世代が分かれ、調停の `conflicted` も入る）の2通り。積む順序は「元の順」「候補の並び順（まとめて1世代／1件1世代）」「逆順」「無作為3通り」の7通り。全ケース・全順序で一致。あわせて、要素が9999個を超えても当て順が変わらないこと（`node_id` の文字列比較への逆戻り防止）、`seq` の単調増加、`conflicted` の記録、一括採用が1世代にまとまること、決め直した候補は後の決定だけが当たることも確かめた。
- 検証: 設計書6章の5コマンドすべて通過し、結果はS1前と同じ。`run-output-tests.js` 111件（S1前94件＋新規17件）、`run-table-tests.js` 7件、`run-parity-tests.js` 223件、`run-tests.js` 正常終了。`npm run test:saga-gold` は指標一致648・差分15でS1前と同じ（`lib/sagaAutoFix.js` は候補の決定を通らないため影響しない）。
- 再レビュー（Fable、承認相当）の申し送り: GOAL1と同じ決定の集合でも佐賀市51ページ中5ページで採用済みの決定に `orphaned` が立つ。畳み込みの対象になる表の候補ではないため、修正が最終HTMLから落ちている可能性がある。原因は一通りではなく、3ページは「先祖の要素ごと差し替え」で説明できるが、2ページは先祖にも同じ `node_id` にも採用済みの候補が無い。S1の範囲外なので直さず、設計書 3.13 のS2に「確かめる項目」として表で書き足した。
- 関連ファイル: `goal2-app/public/app.js`、`goal2-app/test/goal2-output/run-output-tests.js`、`goal2-app/TONO_FEEDBACK_FIX_INSTRUCTIONS.md`
- 関連PR/コミット: PR #131
...
- 主な変更内容(`goal2-app/public/app.js`、`goal2-app/lib/sagaAutoFix.js`):
  - 指摘2: 直前に見出しが無いとき、`dataTableCaptionText()` が1行目のセルを連結して「の詳細」を付けていた。行の中身がそのまま文言になるため、同じ構造の表でもページごとに違うキャプションが提案されていた。1行目連結と「表の詳細」のフォールバックを削除し、見出しから導けないときは文言を空にした。キャプション専用の候補は「文言を調整」で入力してからでないと採用できない。`lib/sagaAutoFix.js` の複製も同じ規則に直し、同名関数の重複宣言（先の宣言が死んでいた）も整理した。
  - 指摘1: h3が4つ並ぶページで先頭の見出ししか直らなかった。飛びの検出より先に、見出し全体を h2 起点へ揃える `shift-headings` の候補を出すようにした。飛びの検出は補正後のレベルで行うため、補正候補と飛びの候補が重ならない。AIへ渡すアウトラインは、見出しを全件、段落を先頭120件までにした。
- 設計書との差異:
  - 4.4: 「データ表として維持し構造を整える」の手段は確信度を下げなかった。下げるとGOAL1の一括採用で代わりに「箇条書きに変換する」が採用され、行と列の関係を持つ表が解体される（安城市の入れ子の表で確認）。「文言を調整でしか採用できない」扱いはキャプション専用の候補に限った。あわせて、キャプションを作らなくなると `quickEditConfig()` が簡易編集を返さなくなるため、`insert-caption` モードの簡易編集を追加した。
  - 4.5: `requiresHumanReview: true` にすれば一括採用から外れる、という設計書の想定は成り立たなかった。`canBulkAcceptCandidate()` は `requires_human_review` を見ていない。この食い違いは今回の変更以前からあり範囲が広いため直さず、`shift-headings` だけを一括採用の対象から外した。
- 検証: 設計書6章の5コマンドをすべて実行した。`run-output-tests.js` 87件（新規16件）、`run-table-tests.js` 7件、`run-parity-tests.js` 223件、`run-tests.js` 正常終了。`npm run test:saga-gold` は指標一致648で変更前と同じ。キャプションは佐賀市の51ファイルでは見出しの文脈からすべて決まっており、1行目連結のフォールバックが使われていない（前後とも21ファイル・81件でgoldと同数）。見出しの変更は `public/app.js` の候補生成だけで、saga-goldが使う `lib/sagaAutoFix.js` は独自の `promoteHeadingsBeforeFirstH2()` を持つため動かない。
- 関連ファイル: `goal2-app/public/app.js`、`goal2-app/lib/sagaAutoFix.js`、`goal2-app/TONO_FEEDBACK_FIX_INSTRUCTIONS.md`、`goal2-app/test/goal2-output/run-output-tests.js`
...
- 主な変更内容(`goal2-app/public/app.js`、`goal2-app/lib/sagaAutoFix.js`):
  ...
  - 指摘12: `alt=""` の装飾アイコンに「画像内容を具体的に入力」を提案していた。`isLikelyDecorativeIcon()` を足し、`alt=""` の装飾アイコンには候補を出さず、`alt` 属性の無い装飾アイコンには `alt=""` を提案するように分けた。レビューの指摘を受けて、画像だけのリンク（`alt=""` にすると名前の無いリンクになる）と、テキスト付きリンクの中の大きな写真は装飾から除いた。さらに、判定を真偽ではなく根拠（寸法・ファイル名・リンクの文脈）を返す形にし、リンクの文脈だけが根拠のときは確認必要にした。寸法を持たない写真が確認不要で空にされるのを防ぐためである。ファイル名の判定は語の強さで3群に分け、群ごとに許す形を変えた。`pdf_thumbnail.jpg` や `new_building.jpg` のような、自治体サイトに普通にある内容画像がファイル名だけで装飾になるのを防ぐためである。
- 既知の残り: 背景色と `table.caption` を両方採用すると、作業中のプレビューには `bgcolor` が残って見える。最終HTMLからは廃止属性の除去で消えるため出力は正しい。候補の `after_html` が決定後に作り直されないためで、設計書の構造変更1（`rebuild` 操作）で解消する。
- 設計書との差異:
...
- 主な変更内容(`goal2-app/TONO_FEEDBACK_FIX_INSTRUCTIONS.md`):
  - 現状の候補のライフサイクル（生成、決定、リプレイ、証跡）を事実として整理し、調停ロジック（`resolveSupersededTableCandidates`、`resolveAlternativeMethodCandidates`）が後追いの埋め合わせであることを明記した。
  - 構造変更1として、決定ログの分離、派生ノードID、`rebuild` 操作、決定のたびの再導出と指紋による照合、排他グループによる代替手段の宣言、5つの実装ステージを設計した。
...
- 背景・目的: 2026年8月に実施した3方式比較（通常移行、AI移行、miChecker移行）の記録が、移行管理シートとAI移行の指摘シートに分かれたままだった。設計に対して何をどこまでやったかを、後から追える形で残した。
- 主な変更内容:
  - `memory/verification-2026-08-summary.md` を追加した。検証の設計、対象ページの準備とCMS取込の結果、作業記録の仕組み、自治体別の実施状況と所要時間、AI移行で出た指摘15件、記録運用の課題、未実施の項目を記載した。
  - 3方式の比較がまだ成立しない理由（通常移行の計測が安城市5件と浦添市49件、AI移行が安城市48件で、同一ページを3方式で作業した組が限られる。浦添市はツール作業の打刻と難易度ランクがない）を明記した。
  - `memory/project-state.md` のファイル一覧に上記の文書を追加した。
- 関連ファイル: `memory/verification-2026-08-summary.md`、`memory/project-state.md`

## 2026-07-28: スライド資料の操作バーに検証作業ガイドへの導線を追加

- 背景・目的: スライドを読み終えた後、手順書へ移る導線が資料の中（12枚目のリンク）にしかなかった。どのスライドを見ていても手順書へ移れるよう、操作バーに置いた。
- 主な変更内容(`goal2-app/public/verification-slides.html`):
  - 操作バーの右端に「検証作業ガイドに戻る」のリンクを追加した。スライド送りの操作と混同しないよう、ボタン群とは離して右端に置いている。見た目はアプリの二次ボタンに合わせた。
  - 左右の`auto`マージンで空きを等分し、スライド送りのボタン群が中央に残るようにした。
  - 操作バーを折り返し可能にした。幅の狭い画面では2行になり、横スクロールが出ない。
- 検証: 幅1400 / 900 / 420pxで、横スクロールが出ないことと配置を確認。リンクから手順書へ遷移すること、キー操作でのスライド送りが引き続き動くことをPlaywrightで確認した。`node test/run-tests.js` 正常終了。
- 関連ファイル: `goal2-app/public/verification-slides.html`
...
- 主な変更内容(`goal2-app/public/verification-slides.html`):
  - `styles.css` の `:root` トークン（色・タイポ・角丸・影・余白・モーション）を写して使うようにした。単体で配布・印刷できる必要があるためリンクではなく複製しており、その旨と「変更時は両方を合わせること」をファイル先頭に明記している。
  - `color-scheme: light` に合わせ、独自に持っていたダークモードの定義を削除した（アプリはライト固定のため）。
  - カードは `--surface` / `--line` / `--radius-lg` / `--shadow-1`、本文は `--font-sans`（Yu Gothic UI）に変更。強調は `--primary` 系、前回の問題点は `--error-container`、注意は `--warning-container` と、意味に対応した既存トークンを使う。
  - 背景をアプリと同じグラデーション＋ゆっくり動く光の層にした（`prefers-reduced-motion` で停止）。
  - 操作ボタンをアプリのボタン階層に合わせ、「次へ」だけを `primary` にした。フォーカスリングも同じ指定にしている。
  - 4つの判断（採用／文言を調整／却下／要確認）を、アプリの決定ボタンと同じ意味の色（success / primary / error / warning）で示すようにした。
  - スライドの高さを `aspect-ratio: 16/9` 固定から「16:9を下限にして内容が多ければ縦に伸びる」形に変更し、内容が枠内で切れないようにした。あわせて縦長のキャプチャに高さの上限を付けた。
  - URLのハッシュを直接書き換えたときにも該当スライドへ移動するようにした。
- 検証: アプリをローカル起動し、Playwrightで12枚すべてを送って内容が枠内で切れないこと、`--primary` とフォントがアプリと一致すること、画像と代替テキスト、横スクロールの有無を確認した。
- 関連ファイル: `goal2-app/public/verification-slides.html`

## 2026-07-28: 佐賀市との違いを説明するスライド資料を追加

- 背景・目的: 検証作業ガイドは細かい手順書のため、作業を始める前に「前回（佐賀市）と何が違うのか」を短く伝える資料が別途必要だった。BPOオペレーター向けに、スライド送り形式のHTMLを用意した。
- 前回との違いの整理: 佐賀市では、AIがアクセシビリティ最適化したHTMLをそのままCMSへ取り込み、取り込んだ後に人が目視で確認していた。今回は、AIが修正候補を提案し、人が目視で確認して最適化HTMLを作ってから取り込む。取り込んだ後の作業は前回と同じで、増えるのは取り込み前の1工程だけである。この違いにより、前回発生していた無駄な修正（AIが直しすぎた箇所を最適化前の状態に戻す作業／AIが拾えていなかった箇所を後から探して直す作業）を、取り込む前に片付けられる。
- 主な変更内容:
  - `goal2-app/public/verification-slides.html`（新規）: 全12枚。2枚目で前回と今回の流れを並べて「人の確認が1工程前になる」ことを示し、3枚目で前回の無駄な修正2種類と、今回それがどう解消されるかを対比する。以降は規模感（佐賀市51ページ・384件・1ページ平均7.5件）、3パターンの担当、作業の流れ、候補レビューの画面3枚、miChecker、証跡、変わらない点とお願い。
  - スライド送り（←→キー・クリック・Home/End）、`S`キーで一覧表示、印刷で全スライドをページ分割してPDF化できる。外部リソースは使わず、既存のCSS変数を流用してダークモードにも追従する。
  - 非表示スライドは`hidden`属性で支援技術からも隠し、現在位置を`aria-live`で通知する。画像は既存の画面キャプチャを流用し、代替テキストとキャプションを付けている。
  - 検証作業ガイドのヘッダー情報欄から、このスライドへリンクした。
- 検証: アプリをローカル起動し、Playwrightでスライド12枚・初期表示1枚・キー操作での遷移・一覧表示・画像の読み込み・代替テキストの有無・横スクロールの有無を確認した。
- 関連ファイル: `goal2-app/public/verification-slides.html`, `goal2-app/public/verification-guide.html`

## 2026-07-28: CMSでの操作手順に画面キャプチャ16点を追加

- 背景・目的: 4章「CMSでの操作手順（データ取込）」は本文のみで、各手順の画面が入っていなかった。実際の操作画面のキャプチャが用意できたため、全13手順に図を付けた。
- 主な変更内容:
  - `goal2-app/public/images/verification/cms-01-login.jpg` 〜 `cms-16-complete.jpg`（新規16点）を追加。提供された18点のうち、重複1点と本手順と無関係の1点（別処理の実行中表示）は除いた。
  - ブラウザのウィンドウ全体で撮られていたため、上下の余分な部分（タブ・アドレスバー・ブックマークバー・タスクバー）を切り落とし、ページの表示領域だけにした。CMSの青いヘッダーは手順の目印になるため残している。
  - 4章の各手順に`<figure>`として図4-1〜図4-16を挿入した。既存の図と同じく、代替テキストとキャプションを付けている。手順9（本文をソースで編集）は3点、手順11（取り込み範囲の更新）は2点。
  - 章冒頭の注記を「画面キャプチャは後日追加します」から「画面は更新される場合があります（差異が出たら実画面を優先）」に変更した。
- 検証: アプリをローカル起動し、Playwrightで画像27点すべてが表示されること、代替テキストの抜けがないこと、4章の図が16点であること、ページ内リンクにリンク切れがないこと、横スクロールが出ないことを確認した。`node test/run-tests.js` 正常終了。
- 関連ファイル: `goal2-app/public/verification-guide.html`, `goal2-app/public/images/verification/cms-*.jpg`

## 2026-07-28: CMSでの操作手順（データ取込）を全13手順に拡充

- 背景・目的: 4章のCMS操作手順はログインから「ページ一括登録」画面を開くまでのプレビュー版だった。データ取込の全手順（13手順）がまとまったため、章の内容を差し替えた。
- 主な変更内容(`goal2-app/public/verification-guide.html`):
  - 4.1「ログインから対象データの確認まで」（手順1〜7）: ログイン、顧客選択、ページビュー、サイト管理、「コンテンツ移行管理」＞「ページ移行」、「ページ一括登録」タブでの移行管理ID検索、詳細でのデータチェックOK・解析OKの確認。
  - 4.2「取り込み範囲の編集と更新」（手順8〜11）を新設。「取り込み範囲編集」画面で本文をソース（HTML）編集し、確認・更新するまで。ツールで取得した最終HTMLをCMSへ戻すのがこの操作であることを明記した。
  - 4.3「データの登録と完了確認」（手順12〜13）を新設。一覧で対象データにチェックを入れて登録し、結果が「完了」になることを確認するまで。
  - 提供資料の変更に合わせて、遷移を「コンテンツ移行管理」＞「ページ移行」＞「ページ一括登録」タブに修正し、移行管理IDの例を `aimig-anjo-002` に更新した。
  - 5章のパターン2手順から、解析済みソースコードの取得とCMS取込ツールへ戻す手順に、4章への参照を追加した。
  - 章冒頭の注記を「プレビュー版」から「画面キャプチャは後日追加」に変更した（手順の流れは確定、スクリーンショットは未反映）。
- 検証: アプリをローカル起動し、Playwrightでページ内リンクにリンク切れがないこと、4章が13手順になっていること、横スクロールが出ないことを確認した。`node test/run-tests.js` 正常終了。
- 関連ファイル: `goal2-app/public/verification-guide.html`
- 関連PR/コミット: PR #116
...
- 主な変更内容(`goal2-app/public/verification-guide.html`):
  - サイドナビゲーションを追加。画面幅1440px以上で左側に固定表示し、`scroll-behavior: smooth`（`prefers-reduced-motion`を尊重）でスムーススクロールする。IntersectionObserverで、いま読んでいる章に`aria-current`を付けて強調する。狭い画面では従来どおりページ冒頭の目次を使う。配色は既存のCSS変数を使い、ダークモードにも追従する。
  - 4章「CMSでの操作手順（データ取込）」を新設。CMSログイン→サイト管理→コンテンツ移行管理→ページ一括登録タブ→移行管理IDで検索、までと、タブ構成の一覧を記載した。提供資料がプレビュー版のため、その旨と「以降は順次追記する」ことを章の冒頭に明記している。時間計測との関係（事前準備は計測しない／取込・残作業は計測する）も9章と対応づけて書いた。
  - 移行管理シート（Google スプレッドシート）へのリンクを、ヘッダーの情報欄・2.3（新設）・4章の3か所に追加し、2.5の準備完了チェックリストにも項目を足した。
  - 章の新設に伴い、旧4〜10章を5〜11章へ繰り下げ、章番号・節番号・アンカー・本文中の「N章」参照を更新した。
- 検証: アプリをローカル起動し、Playwrightで幅1500pxのライト／ダーク表示を確認。ページ内リンクにリンク切れがないこと、画像11点が表示されること、サイドナビと本文が重ならないこと、幅1100pxではナビが出ず横スクロールも出ないことを確認した。`node test/run-tests.js` 正常終了。
- 関連ファイル: `goal2-app/public/verification-guide.html`
- 関連PR/コミット: PR #115
...
- 背景・目的: 手順書のレビューを受けて3点の要望が出た。①ページが長く目的の章まで到達しにくい、②移行管理シートの所在が書かれていない、③CMS側の操作手順がない。
- 主な変更内容(`docs/a11y-review-verification-guide.html`):
  - サイドナビゲーションを追加。画面幅1440px以上で左側に固定表示し、`scroll-behavior: smooth`（`prefers-reduced-motion`を尊重）でスムーススクロールする。IntersectionObserverで現在読んでいる章に`aria-current`を付けて強調する。冒頭は暗色のヘッダーと重なって読めないため、ヘッダーが画面から外れてから表示する。狭い画面ではページ冒頭の目次がそのまま目次として機能する。
  - 移行管理シート（Google スプレッドシート）へのリンクを、ヘッダーの情報欄・3章の準備物の表・5章の入力手順・新設の7章（移行管理ID）の4か所に追加した。
  - 7章「CMSでの操作手順（データ取込）」を新設。CMSログイン→サイト管理→コンテンツ移行管理→ページ一括登録タブ→移行管理IDで検索、までを記載した。提供資料がプレビュー版のため、その旨と「以降は順次追記する」ことを章の冒頭に明記している。2章の全体フロー（STEP 1 / STEP 4〜5）からも参照する。
  - 章の新設に伴い、旧7〜10章を8〜11章へ繰り下げ、章番号・アンカー・相互参照を更新した。版を v1.2 とした。
- 検証: Playwrightで、ページ内リンクにリンク切れがないこと、サイドナビのスムーススクロールと現在位置の強調が動くこと、幅1440/1500/1700pxでナビと本文が重ならないこと、幅1000pxではナビが出ないことを確認した。
- 関連ファイル: `docs/a11y-review-verification-guide.html`

## 2026-07-27: 移行方式の比較検証（通常／AI／miChecker）の作業手順書を画面キャプチャ付きでHTML化

- 背景・目的: 同一ページを「1. 通常移行 / 2. AI移行 / 3. miChecker移行」の3方式で処理し、所要時間と品質を比較する検証を実施する。作業を担当するBPOオペレーターと評価担当が、開発の知識なしで読み進められる手順書が必要だったため、`goal2-app`（アクセシビリティ候補レビュー）の実画面キャプチャを含む手順書をHTMLで整備した。
- 主な変更内容:
  - `docs/a11y-review-verification-guide.html`（新規）: 全10章の手順書。検証の目的と3方式の位置づけ、方式2・3共通の作業フロー（CMS解析→候補レビュー→最適化→取込ツールへ戻す→CMS取込）、事前準備、方式1の扱い、方式2の詳細手順、方式3の手順と「同一性検証」、本文抽出画面、計測・記録項目、FAQ、用語集・画面一覧で構成。
  - 画面キャプチャ9点を `docs/images/a11y-review-guide/`（新規）に配置し、図5-1〜5-5（修正基準の切り替え／入力画面／3列レイアウト／候補の詳細と判断ボタン／出力欄）、図6-1〜6-3（miChecker指摘対応のみの候補一覧／miChecker相当チェック結果パネル／miChecker結果比較画面）、図7-1（本文抽出画面）として本文から参照する。各図には代替テキストとキャプションを付与した。
  - 実施体制の準備（オペレーターの3グループ分け、CMS側の「通常取込 / 解析まで① / 解析まで②」、記録用スプレッドシート）と、対象ページ数の考え方（上限の目安 1自治体50ページ×5自治体＝250ページ）を3章に明記した。
  - 方式3では、ツールのmiChecker判定が公式miCheckerと一致するかを別途検証する必要があるため、「miChecker結果比較」画面を使う2通りの手順（結果CSVの読み込み／ローカルWindowsでの `htmlchecker.exe` 自動比較）を記載した。
...
- 修正1(見出し行の誤判定): 2列の「ラベル / 値」の表で、1行目が`<th scope="col">`のヘッダー行と判定されていた(遺跡番号 / 541031)。原因は`profile.firstRowHeaderLike`が「28文字以下で文末記号が無い」だけで立つため、値が短いだけの行も見出し行に見えること。`hasLabelColumnLayout()`を追加し、2列でどの行も1列目がラベルらしい表は、1行目も「ラベル: 値」のデータ行として扱うようにした。「項目 / 内容」のような定型の見出し語が並ぶ場合は、従来どおり見出し行として扱う。
- 修正2(はみ出したcolspan): 列数を超える`colspan`が、キャプション付与の手段(`buildDataTableSemanticsHtml`)ではそのまま残っていた。`clampOverflowingColspans()`で列数に合わせて詰める。フラット化の手段(`computeTableGridShape`)と同じ考え方を、構造を整える手段にも適用した。
- 修正3(2文字語の均等割り付け): 「墳　丘」「氏　名」のように2文字の語を1文字ずつ空けた表記が未検出だった。既存の`text.spaced-characters`は3つ以上の並びだけを対象にしていたため。両側が単独の漢字・かなで、その外側に別の漢字・かなが続かない場合だけを追加で拾う(「東京　大阪」のような語と語の区切りは対象外)。既存パターンと範囲が重なる場合は追加しない。
- 修正4(畳み込みの取りこぼし): 表のセル内の修正が、入れ子の表を採用した時点で消えていた。外側の表へ畳み込んだ時点でconflictedになった修正が、後から採用した内側の表の変換後HTML(元の要素から作られる)で上書きされていた。conflictedの内容修正候補も、後続の祖先へ畳み込むようにした。
- 検証: `goal2-app/test/goal2-output/run-output-tests.js`に9項目を追加(計38項目)。内蔵サンプル8件で候補数・注意数は変更前と一致。`node test/run-tests.js` 正常終了、miChecker parity 223/223、`test/table-nesting/` 7/7。報告のソースで、中のデータ表が`<th scope="row">遺跡番号</th><td>541031</td>`になり、colspanが消え、「墳　丘」が「墳丘」に詰まることを確認。
- 関連ファイル: `goal2-app/public/app.js`, `goal2-app/test/goal2-output/run-output-tests.js`
- 関連PR/コミット: (このコミット、PR #123)

## 2026-07-29: 表の中にある表が誰も直さないまま残る不具合を修正

- 背景・目的: 「最初のテーブルが修正しきれてないよね」との指摘。外枠の表を解体しても、その中にあった表(市指定史跡/遺跡番号)が元のまま最終HTMLへ残っていた。
- 原因1(再評価されない): 表構造候補を採用すると、`resolveSupersededTableCandidates()`が表内の表関連候補を一律で「変換後HTMLで再評価するものとして自動解決」していた。しかし再評価の実装は無く、親の変換後HTMLにはセルの中身がそのまま出るため、入れ子の表は形を変えずに残る。その表への候補(キャプション・行見出し・フラット化・解体)がすべて解決済みになり、誰も直さないまま出力されていた。対象が親の変換後HTMLに残っている場合は自動解決せず、未処理のままにするようにした。
- 原因2(対象を見つけられない): 親の要素を置き換えると、中にあった表の`data-goal2-node-id`も一緒に消えるため、候補を残しても適用できなかった。`replaceTarget()`で、置き換え前に中にあった表のIDを、置き換え後の同じ並びの表へ引き継ぐようにした。
- 原因3(表がテキストへ潰れる): `isContentBlockElement()`にTABLEが含まれておらず、セルの中の表が「インラインの中身」として扱われていた。そのためセル全体が1つの見出しへ潰れ、`<h3>遺跡番号 541031 墳 丘 円墳</h3>`のように行と列の対応が失われていた。TABLEをブロック要素に含め、表は表のまま出力するようにした。あわせて、表を含むセルを見出し候補として扱わないようにした。
- 結果: 外枠の1列の表は`<h3>市指定史跡</h3>`になり、中のデータ表は表として残ったうえで、その表に対するキャプション・行見出し・scope属性の候補が適用されるようになった。
- 検証: `goal2-app/test/goal2-output/run-output-tests.js`に3項目を追加(計29項目)。解体後も入れ子の表が表として残ること、見出しのテキストへ潰れないこと、行と列の対応が失われないことを確認。内蔵サンプル8件で候補数・注意数は変更前と一致。`node test/run-tests.js` 正常終了、miChecker parity 223/223、`test/table-nesting/` 7/7。
- 残件: 中のデータ表に対する提案の質は未対応。2列の「ラベル/値」の表で1行目が`<th scope="col">`のヘッダー行と判定されること、列数を超えた`colspan="2"`がキャプション付与の手段では残ること、表セル内の全角空白(「墳　丘」)が検出対象外であること。
- 関連ファイル: `goal2-app/public/app.js`, `goal2-app/test/goal2-output/run-output-tests.js`
- 関連PR/コミット: (このコミット)

## 2026-07-27: 同じ段落にある独立した修正を、代替手段として見せないようにした

- 背景・目的: 「単位とか文字間空白が同じパラグラフ内だったら同じ箇所扱いされてしまうのは仕方ないのかな？プレビューもそうだけど」との指摘。候補は`target.node_id`(要素)しか位置情報を持たないため、1つの段落にある単位の言い換え3件と単語内空白の除去2件が「同じ箇所の代替手段 5件中」と表示され、プレビューも段落まるごとがハイライトされていた。どれを直す候補なのかが分からない。
- 主な変更内容(`goal2-app/public/app.js`):
  - `alternativeMethodCandidates()`を追加し、代替手段を「同じ要素を指し、どちらも要素ごと差し替える候補」に限定した。要素を残す候補(単位の言い換え・単語内空白の除去など)は、それぞれ独立した修正として扱う。
  - 候補一覧のグループを2つに分けた。要素ごと差し替える候補は「同じ箇所の代替手段・N件」(いずれか1つを選ぶ)、要素を残す候補は「同じ箇所の修正・N件」(どれも必要)。「同じ箇所の代替手段 N件中」のバッジも、本当の代替手段にだけ付ける。
  - 詳細ペインの「修正方法」一覧を代替手段だけにした。単位の言い換えを選んでも、同じ段落の他の修正が選択肢として並ばない。
  - 候補のタイトルに置換内容を添えた(`単位の表記：22m → 22メートル`)。1つの段落に同じルールの候補が複数あっても見分けがつく。
  - プレビューのハイライトを文字単位にした。文字列置換の候補は置き換える文字だけを`<mark>`で強調し、一致するテキストが見つからない場合は従来どおり要素をハイライトする。
- 検証: `goal2-app/test/goal2-output/run-output-tests.js`に6項目を追加(計26項目)。グループの分かれ方、バッジの有無、タイトルの中身、修正方法が1件になること、プレビューが「22m」だけを強調することを確認。内蔵サンプル8件で候補数・注意数は変更前と一致。`node test/run-tests.js` 正常終了、miChecker parity 223/223、`test/table-nesting/` 7/7。
- 関連ファイル: `goal2-app/public/app.js`, `goal2-app/test/goal2-output/run-output-tests.js`
- 関連PR/コミット: (このコミット、PR #122)

## 2026-07-27: 同じ要素に複数の修正がある場合に、1件しか反映されない不具合を修正

- 背景・目的: 「22mの単位が修正されない」「全　長の単語内空白が修正されない。同じ箇所での指摘はあるが修正が反映されない」との報告。1つの段落に単位の言い換えが3件、単語内空白の除去が2件あっても、最終HTMLにはどれも反映されていなかった。表の有無に関係なく再現する、以前からの不具合。
- 原因1(パッチを当てる順序): 同じ要素を指す候補を候補一覧の順に当てていた。装飾タグの解除(`unwrap-element`)が先に当たると、要素と一緒に`data-goal2-node-id`も消えるため、後続の`replace-text`が対象を見つけられず`applyCandidatePatch`が黙ってreturnしていた。`rebuildWorkingHtmlFor()`で、同じ要素については要素を残すパッチを先に、要素ごと差し替えるパッチを後に当てるようにした。
- 原因2(表の中の修正の畳み込み): `foldDescendantFixIntoAncestor()`が要素まるごとの`before_html`→`after_html`で畳み込んでいたため、同じ要素に複数の修正があると2件目以降が「元の要素」を見つけられず捨てられていた。`replace-text`のパッチは置換する部分だけを畳み込むようにした。あわせて、`decision.after_html`が未設定の場合に`proposal.after_html`を起点にするようにした(ヘッドレス経路の`autoAcceptSafe`は`after_html: null`のまま採用するため、畳み込みが一度も成立していなかった)。畳み込む順序も、要素ごと差し替えるものを先にした。
- 原因3(NBSPの表記の違い): パッチが持つのはDOMのテキスト(実際のNBSP文字)、畳み込み先は直列化されたHTML(`&nbsp;`)で、文字列が一致しなかった。単語内空白の除去のように`&nbsp;`を含む修正が畳み込めていなかった。`serializeTextForHtml()`で同じ直列化を通してから突き合わせるようにした。
- 前回の修正の見直し: `resolveAlternativeMethodCandidates()`が、同じ要素を指す候補をすべて「代替手段」として自動解決していた。単位の言い換えのように要素を残すパッチは1つの段落に何件あっても互いに独立して当たるため、対象を「どちらも要素ごと差し替える候補」に限定した。
- 主な変更内容(`goal2-app/public/app.js`): `isElementReplacingCandidate()`と`serializeTextForHtml()`を追加。`rebuildWorkingHtmlFor()`・`resolveSupersededTableCandidates()`・`foldDescendantFixIntoAncestor()`・`resolveAlternativeMethodCandidates()`を修正。
- 検証: `goal2-app/test/goal2-output/run-output-tests.js`に5項目を追加(計20項目)。段落単体・表の中の両方で、単位の言い換え3件・単語内空白の除去2件・装飾タグの解除がすべて反映されることを確認。内蔵サンプル8件で候補数・注意数は変更前と一致。`node test/run-tests.js` 正常終了、miChecker parity 223/223、`test/table-nesting/` 7/7。報告のソースで「南北約22メートル、東西17.5メートル」「全長：」「高さ：」になることを確認。
- 関連ファイル: `goal2-app/public/app.js`, `goal2-app/test/goal2-output/run-output-tests.js`
- 関連PR/コミット: (このコミット)

## 2026-07-27: 候補の排他性・レイアウト表への提案・最終HTMLの後始末・連番altの検出を修正

- 背景・目的: 安城市の史跡ページの報告で洗い出した残りの4件(同じ表に排他でない手段が並ぶ、一括採用が競合候補を同時に採用する、CMS独自タグや廃止属性・空段落が残る、連番altが未検出)に対応した。
- 修正1(一括採用が競合候補を同時に採用する): `table.simple-structure`が`tableStructuralRuleIds`・`tableRelatedRuleIds`のどちらにも登録されておらず、表を丸ごと置き換える手段どうしの競合解決が働いていなかった。両方へ登録した。あわせて`resolveAlternativeMethodCandidates()`を追加し、同じ`target.node_id`を持つ候補(画面上の「同じ箇所の代替手段」)は1つ採用した時点で残りを自動解決するようにした。どの候補も変換後HTMLを元の要素から作るため、2つ以上採用すると後から当てた方が前の修正を丸ごと上書きし、出力が適用順で決まっていた。
- 修正2(ヘッドレス経路): `goal2Engine.autoAcceptSafe()`(GOAL1の一括最適化が使う)が競合解決を通さず、決定を直接書き込んでいた。実データで1つのノードに5件が採用されていた。画面側と同じ解決を通すようにした。
- 修正3(レイアウト表への提案): セルの中に見出しを含む表には、データ表向けの手段(キャプション・列見出し・行見出し・scope属性の追加)を出さないようにした。見出しをセルに入れている表はページの段組みであり、行と列の関係を持つデータ表ではない。判定には`isLikelyLayoutTable()`を使わない。`border="0"`とセル内のブロック要素だけで成立するため、「遺跡番号 / 541031」のような正当な2列のデータ表まで対象になってしまう。
- 修正4(最終HTMLの後始末): `stripMigrationUnneededAttributes()`を拡張し、CMS独自タグ(`<ikkr_*>`、中身を残して外側だけ外す)、廃止属性(`align`/`valign`/`border`/`cellpadding`/`cellspacing`/`bgcolor`/`background`/`hspace`/`vspace`/`nowrap`)、レイアウト目的の空段落、見出し・段落の先頭に残った空白を落とすようにした。`width`/`height`は画像の表示サイズとして意味を持ちうるため対象外(image.display-widthの注意で扱う)。
- 修正5(連番altの検出): `isSerialOrFilenameAlt()`を追加し、`alt="碧海山古墳002"`のような連番・ファイル名だけの代替テキストを`image.alt-text`の候補として検出するようにした。誤検出を避けるため、末尾の連番は「数字の前が2文字以上」または「4桁以上」の場合のみ拾う(「図1」のような短い図版番号は対象外)。
- 追加(`goal2-app/test/goal2-output/run-output-tests.js`): 上記4件の回帰テスト15項目(`npm run test:goal2-output`)。
- 検証: 内蔵サンプル8件すべてで候補数・注意数が変更前と完全一致。`node test/run-tests.js` 正常終了、`node test/michecker-parity/run-parity-tests.js` 223/223 passed、`test/table-nesting/` 7/7、`test/goal2-output/` 15/15。報告のあったソースで、レイアウト表へのキャプション案が消え、一括採用で同じ箇所へ2つ以上の候補が採用されなくなり、最終HTMLから`<ikkr_textcenter>`・`align`・空段落・先頭の空白が消えることを確認した。
- 補足: 表の中にある画像のalt候補は、表構造候補を採用したときに変換後HTMLへ畳み込まれる(2026-07-24の対応)。このため、AIの下書きもキャプションも無い場合は`alt="画像内容を具体的に入力"`という記入用の文言が最終HTMLへ入る。altが未設定だった場合の既存の挙動と同じで、証跡には要人間確認として残る。
- 関連ファイル: `goal2-app/public/app.js`, `goal2-app/test/goal2-output/run-output-tests.js`, `goal2-app/package.json`
- 関連PR/コミット: (このコミット)
...
- 主な変更内容(`spreadsheet-gas/`):
  - `Code.gs`: シート名と列番号のハードコードを廃止し、2行目の見出しの文字列から列を解決する方式にした。見出しの改行・空白は無視し、同名の見出しがある場合は左端の列を採用する(既存シートに`差戻し内容(ｽﾀｯﾌ宛て)`などの重複があるため)。対象シートの判定もシート名ではなく、計測列の有無で行う。データ行は3行目からとし、見出し行を選んだまま開始しようとした場合は拒否する。
  - `Code.gs`: 計測を`tool`(ツール作業)と`cms`(CMS作業)の2区間に分け、区間ごとに開始・終了・休憩・作業時間を記録する。終了のたびに両区間の作業時間を合計して`作業時間集計`へ書き込む。ツール作業の列が無いシート(通常移行)ではCMS作業だけが選べる。
  - `Index.html`: サイドバーに区間の選択を追加(計測中は切り替え不可、シートに列が無い区間は選択不可)。記録先の表示をシート名・行・区間に拡張し、ガイドのタブも新しい仕様に書き換えた。
  - 不具合修正: 休憩したまま終了すると、その休憩分が実働時間に含まれていた。終了処理で休憩を締めてから集計するようにした。
  - 不具合修正: 累計再開のときに、シートの休憩時間を読み込んで差し引いていた。既存の作業時間は休憩を差し引いたあとの値のため、二重に引かれていた。休憩時間は読み込まないようにした。
  - 不具合修正: 「新規(上書き)で開始」でも休憩時間・作業時間・終了時刻の既存値が残っていた。開始前にその区間の値を消すようにした。
  - 不具合修正: タスク切り替えのモーダルをキャンセルすると、サーバー側は終了済みなのに画面が「作業中」のままになっていた。終了後に待機中へ戻すようにした。
  - 不具合修正: 終了時の`deleteAllProperties()`が、このスクリプト以外のユーザープロパティも消していた。自分のキーだけを消すようにした。
  - `README.md`: シートの作り方(見出しは2行目、データは3行目、計測に使う9つの見出し)、使い方、動作確認の内容を記載。
- 検証: Apps Script上でしか実行できないため、GASのAPI(SpreadsheetApp・PropertiesService)をモックしてロジックを確認した(30項目)。列の解決、2区間の計測と合算、休憩の扱い、累計再開、上書き開始、計測中の検知、中断と再開、プロパティの消し方を確認済み。
- 関連ファイル: `spreadsheet-gas/Code.gs`, `spreadsheet-gas/Index.html`, `spreadsheet-gas/README.md`
- 関連PR/コミット: (このコミット)

## 2026-07-27: 記録用スプレッドシートのGASを置く場所を用意

- 背景・目的: 移行作業の記録用スプレッドシートで以前使用していたGoogle Apps Scriptを改修するにあたり、まずそのスクリプトをリポジトリへ取り込みたい。既存スクリプトを貼り付けて上書きするための入れ物として、ディレクトリとファイルを作成した。
- 主な変更内容(`spreadsheet-gas/`):
  - `Code.gs`: スクリプト本体の入れ物。上書き前提であることを冒頭のコメントに書いてあるだけで、実行しても何も起きない。
  - `appsscript.json`: マニフェスト。タイムゾーン(Asia/Tokyo)と実行環境(V8)のみ。
  - `README.md`: ファイル構成、Apps Scriptエディタからの取り込み手順、claspを使う場合の注意、リポジトリへ置かないもの(スクリプトID・スプレッドシートID・実データ)を記載。
  - `.gitignore`: `clasp clone`/`clasp pull` で生成される`.clasp.json`を除外(スクリプトIDが入るため)。
- 補足: 中身は未取り込みのため、既存スクリプトの内容で上書きする作業が次に必要になる。
- 関連ファイル: `spreadsheet-gas/README.md`, `spreadsheet-gas/Code.gs`, `spreadsheet-gas/appsscript.json`, `spreadsheet-gas/.gitignore`
- 関連PR/コミット: (このコミット)

## 2026-07-27: 検証作業ガイドのグループ分けと、miChecker一致確認の時間の扱いを修正

- 背景・目的: グループ分けの意図と、miChecker本体との結果一致確認の時間の扱いについて指摘を受けた。オペレーターを3グループに分けるのは、各グループが3パターンをそれぞれ担当するためであり、担当パターンをローテーションする想定ではない。また、一致確認は計測時間に含めない。
- 主な変更内容(`goal2-app/public/verification-guide.html`):
  - 「2.1 BPO：オペレーターを3グループに分ける」のローテーション案(ラテン方格の割り当て表と「提案・要合意」の注記)を削除し、グループ1が通常移行、グループ2がAI移行、グループ3がmiChecker移行を担当する対応表に置き換えた。使用するCMSの準備物(通常取込・解析まで１・解析まで２)も同じ表に並べ、2.2の準備物と対応づけた。
  - 所要時間の差にはグループごとの習熟度の差も含まれる点を、集計時の注記として1文だけ残した。
  - 「6章 追加検証」の注記から「別途『検証時間』として記録します」を削除し、計測に含めないことだけを述べる形にした。8.1の計測対象外の表も同じ理由(移行作業ではないため)に揃えた。
- 検証: 幅1100pxで横スクロールが出ないこと、画像11点の表示とページ内リンクに欠落が無いことをPlaywrightで確認。`node test/run-tests.js` 正常終了。
- 関連ファイル: `goal2-app/public/verification-guide.html`
- 関連PR/コミット: (このコミット)

## 2026-07-27: 検証作業ガイドからソースリポジトリのリンクを削除し、3パターンの定義を図にした

- 背景・目的: ガイドの冒頭に載せていたソースリポジトリのURLは伏せたい情報だという指摘を受けた。あわせて「1.2 3パターンの定義」が表と文章だけで、3パターンの関係が読み取りにくかった。
- 主な変更内容(`goal2-app/public/verification-guide.html`):
  - 冒頭のメタ情報から「ソース」の行(GitHubリポジトリへのリンク)を削除。ページ内に他の記載は無いことを確認済み。
  - 「1.2 3パターンの定義」の表を、工程の流れを示す図に置き換えた。同一の対象ページを起点に3パターンへ分岐し、最後に所要時間と品質を比較する構造を示す。各パターンはカードにし、工程を上から順に並べた。
  - ツールを使う工程(パターン2・3)は枠を強調し、「修正基準」の選択値をカード内に併記した。パターン3だけの追加検証(miChecker本体との結果一致確認)は破線の枠で区別し、移行作業の所要時間に含めないことを凡例に明記した。
  - 表に入れていた「位置づけ」「ツールの使用」「修正基準」はカード内へ移し、表は削除した(同じ内容の重複を避けるため)。
- 検証: 幅1100px・390pxの両方で横スクロールが出ないこと、ダークモードでも配色が保たれること、ページ内にリポジトリへの参照が残っていないことをPlaywrightで確認。`node test/run-tests.js` 正常終了。
- 関連ファイル: `goal2-app/public/verification-guide.html`
- 関連PR/コミット: (このコミット、PR #112)
...
- 主な変更内容(`goal2-app/public/app.js`):
  - `resolveSupersededTableCandidates()`を改修。表内の子孫候補のうち、`table.*`以外の「内容修正」候補（file.file-display-text、text.* 等）は、解決扱いにする前に、その修正内容を採用済み表候補の変換後HTML（decision.after_html）へ**畳み込む**ようにした（`foldDescendantFixIntoAncestor()`を新設。子孫の before_html が変換後HTMLに現れる場合に after_html へ置換）。既に採用済みの内容修正候補も、表の丸ごと置換で上書きされないよう畳み込む。畳み込めない場合は未処理のまま残し、修正が失われたまま誤って「完了」になるのを防ぐ。
  - 表関連（th/scope・キャプション・セル結合など、`table.*`）の子孫候補は、表の構造変換で意味がなくなるため従来どおり自動解決。
- 検証: `node --check public/app.js`成功。`node test/run-tests.js`全テスト成功。Playwrightで実UIを操作し、安城市サンプルで表分解候補4件を採用すると、file-display-text 12件が解決扱いになると同時に、出力の「（PDF：」表記が12→0になる（修正が確実に反映される）ことを確認。
...
- 追加ルール `a11y-migration-kb/rules/text/meaningless-symbol.md`（新規, WCAG 1.3.1）:
  - 矢印記号（→↓↑←⇔⇒）は読み上げで読み飛ばされるため、文脈に応じて「から」「へ」等の言葉に置き換える（機械的に「から」へ固定しない）。
  - 装飾記号（○●◎□■◇◆☆★）は削除する（必要なら半角スペースに）。箇条書き・見出しの代わりならリスト・見出しに変換し、固有名詞の一部（例:わくわく☆イベント）は残す。
- GOAL2実装 `goal2-app/public/app.js`:
  - `collectMeaninglessSymbolCandidate()`を新設し、本文テキストノードから矢印・装飾記号を検出。置換先や削除可否が文脈依存のため、自動修正せず確認候補（patchMode: none, advisory notice）を出す。矢印のみ・装飾のみ・両方でメッセージを出し分ける。表内（td/th）の矢印・記号は表ルール側で扱うため対象外。`noticeRuleIds`へ登録。
  - 既存の疑似リスト検出（`・`のみ対象）や表内の記号処理とは重複しないことを確認。
- 検証: `node --check public/app.js`成功。`node test/run-tests.js`全テスト成功。Playwrightで、矢印・装飾記号・両方で確認候補が出ること、通常文・表内・中黒（`・`）では出ないこと（重複なし）を確認。
- 関連ファイル: `a11y-migration-kb/rules/text/meaningless-symbol.md`, `a11y-migration-kb/build/rules.jsonl`, `goal2-app/data/rules.jsonl`, `goal2-app/public/app.js`

## 2026-07-24: 複雑画像の画像名で「詳細は以下」の前を100文字の枠まで具体化するよう改善

- 背景・目的: 実データで vision が「グルメと絶景を楽しむ観光地特集のポスター 詳細は以下」を生成した。主題は具体化されたが約24文字で100文字の枠に余裕があり、ユーザーから「もう一歩」と要望。is_complexがtrueで「詳細は以下」を付ける場合でも、枠が許す範囲で画像内の主要な文字や主要な被写体まで盛り込むべき。
- 主な変更内容:
  - `goal2-app/lib/llm-prompts.js`の`image-alt`タスク: is_complexがtrueの場合、alt_textには主題に加えて画像内の主要な文字・主要な被写体を、代替テキスト一行＝100文字程度に収まる範囲でできるだけ盛り込むよう指示を強化（短いラベルで済ませず使える文字数を活かす）。例を「海に浮かぶ鳥居や五重塔、海鮮丼やラーメン、もみじ饅頭などを紹介する観光地特集のポスター」に更新。100文字にも入りきらない網羅的な詳細のみ complex_detail へ回す方針は維持。
  - `a11y-migration-kb/rules/image/complex-image-report.md`: 「詳細は以下」の前は短いラベルで済ませず、100文字の枠が許す範囲で主要な被写体・文字まで盛り込むと明記。ポスターの例を主要な被写体入りの具体例に更新。
  - `a11y-migration-kb/build/rules.jsonl`・`goal2-app/data/rules.jsonl`を再生成。
- 検証: `node --check lib/llm-prompts.js`成功。`node test/run-tests.js`全テスト成功。実際のGemini出力の変化はローカルにAPIキーが無いため未検証。
- 関連ファイル: `goal2-app/lib/llm-prompts.js`, `a11y-migration-kb/rules/image/complex-image-report.md`, `a11y-migration-kb/build/rules.jsonl`, `goal2-app/data/rules.jsonl`
...
- 主な変更内容:
  - `a11y-migration-kb/rules/image/complex-image-report.md`: 「詳細は以下」の前の主題部分を、分類語一語で終わらせず①主題＋②様子＋③+α情報の考え方でできるだけ具体的に書くと明記。網羅的な詳細（全数値・全項目・全写真）だけを本文追記・報告欄へ回すと整理。例を刷新（グラフ「人口推移の集計結果を示す折れ線グラフ 詳細は以下」、ポスター「絶景スポットとご当地グルメを紹介する観光地特集のポスター 詳細は以下」、150字超の分離例も具体的な主題を残す形に）。
  - `goal2-app/lib/llm-prompts.js`の`image-alt`タスク: is_complexがtrueでも alt_text は分類語一語で終わらせず主題・内容を具体的に説明するよう指示を変更（前回の「短い分類・主題ラベルのみ」を撤回）。網羅的な詳細は complex_detail へ回す方針は維持。あわせて残っていた「alt一行(30文字程度)」の記述を「代替テキスト一行」に整理。
- 検証: `node --check lib/llm-prompts.js`成功。`node test/run-tests.js`全テスト成功。rules.jsonl再生成で複雑画像の例3件が反映されることを確認。実際のGemini出力の変化はローカルにAPIキーが無いため未検証（デプロイ後の再確認を推奨）。
- 関連ファイル: `a11y-migration-kb/rules/image/complex-image-report.md`, `a11y-migration-kb/build/rules.jsonl`, `goal2-app/data/rules.jsonl`, `goal2-app/lib/llm-prompts.js`
...
- 主な変更内容(`goal2-app/public/app.js`):
  - `enrichOneImageGroup()`で、`/api/llm/image-alt`が`ok:false`を返した場合、`llm_not_configured`(GEMINI未設定=意図的なオフの範囲)以外のエラー(画像取得失敗・非対応形式・サイズ超過・タイムアウト・モデルエラー等)については、対象候補(image.alt-text / image.complex-image-report)の理由に`（この画像はAIで解析できなかったため、下の画像名はキャプション・ファイル名等からの機械的な下書きです。AI画像解析エラー: <理由>）`を追記するよう変更。通信エラー(catch)時も同様に追記。
  - これにより、テキスト系LLMが動いていても画像visionだけ失敗するケースで、失敗事実と具体的なエラー内容が候補上に可視化され、原因調査が可能になる。
- 検証:
  - `node --check public/app.js`成功。`node test/run-tests.js`全テスト成功。
  - Playwrightライブ検証: 旧ページURLを設定し解決不能な画像ホストを指定した場合、候補の理由に「AI画像解析エラー: Host could not be resolved」が表示されることを確認(GEMINI未設定時のサイレント維持も従来どおり)。
- 関連ファイル: `goal2-app/public/app.js`
