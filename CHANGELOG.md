...
- 主な変更内容: 表区切り3行を追加。B02・B03・B05・C02・C04・D01〜D04と本文抽出の根拠を更新。S2の未マージ状態と5自治体検証の未実施分を明記。D03・D04の旧記述は原本に存在することを照合したうえで現在の残課題へ更新。参照を相対リンクに変更。
- 関連ファイル: `memory/project-issue-candidates.md`、`CHANGELOG.md`
- 関連PR/コミット: PR #133のマージ後レビューへの対応（本変更のコミット `docs: address PR 133 review`）。
