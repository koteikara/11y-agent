...
- 移行元CSVと移行後CSVを、行番号に頼らずどう機械的に対応づけるか(`JIS`+`達成方法`+`内容`定型部分のシグネチャ案は上記参照、要検証)。
- miCheckerの指摘分類と、`a11y-migration-kb/` のルール分類をどう対応づけるか。
- `htmlchecker.exe`を実際にビルド・実行し、出力CSVの列構成がGUI版のエクスポート形式と同一かを検証する(未検証)。
- `htmlchecker.exe`をどのWindows環境(ユーザーのPC/専用VM等)で動かす運用にするか、goal2-appからその結果CSVをどう受け渡すか。
