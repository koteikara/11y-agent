...
結論: miCheckerはCLI/APIを持たないWindows専用GUIツールであり、goal2-appのプログラムへの直接組み込みは不可能。前後比較は人がGUIで2回実行し証跡を記録する手動ゲートとして扱うのが妥当。
...
- ~~miCheckerを自動実行できるCLI/APIがあるか、またはGUI前提で手動ゲートにするか。~~ → 2026-07-06確認: 開発環境準備手順書によりCLI/APIは無いことを確認済み。GUI前提の手動ゲートとして扱う。
