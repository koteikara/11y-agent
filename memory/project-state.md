...
- Goal 2で、colspanを使った案内文+リンクの結合セル(総合評価方式の入札案件など)を、colspanを使わず表の外へ抽出する「案A」パターンに対応した。
  - `classifyMergedCellTable`に、結合セルがリンク1件と「ご覧ください/ご確認ください」等の案内文を含むケースの検出を追加し、`table.cell-merge-summary`として分類するようにした。
  - 該当行を表から削除し、見出し・段落(実在する列見出しを使用)・リンク付きリスト(`<h3><p><ul><li><a>`)として表の直後に出力する`buildRowExtractedToListHtml`を追加した。
