...
- Goal 2・Goal 3のUI/デザイン評価(2026-07-05実施、実機確認済み)で見つかった次の課題は、ユーザー判断により今回は未修正。
  - Goal 3の「抽出位置の確認」プレビューで、ハイライトされる「抽出対象」範囲に、実際には除外されているパンくず・h1・署名ブロックなどが含まれてしまい、抽出根拠(除外件数)や本文プレビュー(最終HTML)と食い違う。作業者が「ハイライト範囲=CMS登録内容」と誤解する恐れがある。
  - Goal 3で、最終的なクリーニング後HTMLが同一内容でも、`hasMultipleSubstantiveChildren`等のスコアボーナスにより、より広いスコープ(`body`)が`main`/`section`より「おすすめ」候補に選ばれることがある。結果は同じだが、作業者の信頼感を損ねる可能性がある。
  - Goal 2/Goal 3共通の「次にやること」フローティングパネル(`page-agent-panel`)が、閉じるボタンを持たず、位置調整もポインタ操作(`pointerdown`/`pointermove`)のドラッグのみでキーボード代替手段がない。ドラッグアイコンも`aria-hidden="true"`のためスクリーンリーダーから認識できない。アクセシビリティ支援ツール自身がWCAG 2.1.1(キーボード)・2.5.7(ドラッグ操作)の観点で課題を抱えている。
