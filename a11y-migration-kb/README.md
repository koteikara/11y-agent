...
| `wcag` / `jis` | 関連達成基準 |
| `origin` | ルールの出自。`kb`（本KBの独自知見、省略時のデフォルト）／`michecker`（miChecker公式チェックエンジン由来）。同一の問題に対しKB版とmiChecker版が別ファイルで存在する場合は `related` で相互リンクする |
...
- before/after 例: 80件
- うち`origin: michecker`（miChecker公式チェックエンジン由来）: 8ルール
