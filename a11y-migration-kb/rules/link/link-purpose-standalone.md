...
# 必須ルール
本ルールは`link-text.md`（マニュアル版、リンクテキストの文脈非依存化）に含まれる、miChecker側で機械的に検出される具体的な失敗パターン。`link-text.md`の基準を満たしていれば通常は本ルールも同時に解消するが、以下は個別に確認する。
...
```
ポイント: `link-text.md`（マニュアル版）は「指示語だけのリンクを避ける」という編集面の指摘、本ルール（miChecker版）は「空リンク・重複テキスト」というmiChecker側の機械的な検出観点。マニュアル版のチェックが本ルールを内包するため、通常はマニュアル版の基準を満たす作業の中であわせて解消される。
