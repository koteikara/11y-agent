...
**`orphaned` の意味**。畳み込みが無くなったので、S2の `orphaned` は本来の意味、つまり「その決定の操作が当たらなかった」になった。ただし `patch_mode` が `"none"` の候補（通知だけの候補）も、対象が見つからなければ印が立つ。HTMLは元から変えない候補なので「修正が失われた」わけではない。画面に出すS4で、この区別を付けるか判断する。S4 で `no-op`・`target-replaced`・`lost` の3つに分けた（3.11 の S4）。
...
- 「再確認」のバッジは、未処理で `generation` が 0 より大きい候補（決定のあと再導出で生まれた候補）に付け、`aria-label` にも「再確認」を入れる。前の候補から引き継いだ候補は `inheritIntoFresh()` が `generation` を保つので付かない。決定済みの候補にも付けない（再導出で生まれた候補も、決定すると外れる）。
- 取り下げた候補は一覧から消したまま、候補一覧の下の折りたたみ（`details`）「取り下げた候補 N件」に並べる。各行は候補の表示名（`candidateDisplayTitle()`）と原因の決定である。原因は `withdrawn_by_seq` の決定の候補名と決定の種類で「「下線・打消し線・斜体」の採用の後」のように示し、原因の決定と同じ世代に決定が2件以上あるときは「一括採用 N件の後」とする。取り下げの行も原因の決定と同じ世代に積まれるので、N には数えない。この欄の候補はボタンにしないので、選択も決定もできない。開閉の状態は描き直しても保つ。
- 最新の決定の `orphaned` が `lost`（3.11 の S4）の候補には「最終HTMLに未反映」のバッジを付け、`aria-label` にも入れる。候補一覧の上の要約に「最終HTMLに未反映 N件。」を出す。詳細欄には「最終HTML」の行を足し、3分類それぞれの説明を1文で出す。`no-op` と `target-replaced` は「修正は失われていない」ことが分かる書き方にした。
- **`lost` が残っていても完了判定は変えない**。本文の規則（現在の候補一覧に未処理が無い）のままである。`lost` の決定は作業者が採用を済ませたもので、作業者の判断は済んでいる。完了を止めると、画面から直す手段が無い場合（対象の要素がもう無い）に完了できなくなる。
- `bulkActionStatus` の「決定により候補を N件追加、M件取り下げしました。」と、「同じ箇所の代替手段 N件中」の数え方は S3 のままである。
...
- `recordWithdrawal()` が、取り下げた時点の候補の写しを `state.withdrawnCandidates`（`candidate_id` → 写しとログの行）に残す。ログの行は `before_html`・`after_html`・`category` などを持たないので、写しが無いと証跡に並べられない。`resetDecisionLog()` で空にする。`candidate_id` は再利用しないので、同じ候補が2回取り下げられることは無い。
- 証跡の `candidates` は「現在の候補一覧」の後ろに「取り下げた候補」を取り下げの `seq` 順に並べる。取り下げの行は `status: "withdrawn"` で、`decision_reason`・`actor`・`decided_at` はログの行から、`before_html`・`after_html` などは写しから取る。
- 「ログ上の決定済み」は、`reconcile()` が決定済みの候補を一覧に残す（3.6 の5）ので、現在の一覧で満たされている。回帰テストで確かめた。
- `completion.total` と `completion.unresolved` は今までどおり現在の一覧だけを数え、`completion.withdrawn`（取り下げた件数）を足した。
- 候補の行に5つの列を足した。JSON はキー、CSV は既存の23列の後ろにこの順で置く。既存の23列は名前・順序・値を変えていない（同じ操作手順で `main` と比べ、取り下げ以外の行の先頭23列がファイル比較で一致することを確かめた）。

| 列 | 値 | 値を持つ行 |
| --- | --- | --- |
| `generation` | 候補が生まれた再導出の世代。初回の候補は 0 | すべて |
| `decision_seq` | その候補の最新の決定の `seq`（`latestDecisions()` で引く） | 決定のある行。取り下げの行は取り下げの `seq` |
| `withdrawn_by_seq` | 取り下げの原因になった決定の `seq` | 取り下げの行 |
| `orphaned` | 最新の決定がリプレイで当たらなかったか | 採用・編集の行 |
| `orphaned_kind` | 下の3分類 | `orphaned` が真の行 |

- 値を持たない行は、JSON では `null`、CSV では空にする。真偽値は既存の `requires_human_review` と同じく CSV で `true`／`false` と書く。
- **`decision_seq` は本文に無い列で、設計側の判断で足した**。取り下げの原因の候補を、候補の行から引けるようにするためである。
  - 原因の決定は、`decision_log` のうち `seq` が `withdrawn_by_seq` と同じ行で引く。
  - 候補の行の `decision_seq` はその候補の最新の決定なので、原因の候補を決め直すと値が変わる。そのときは候補の行からは引けない（構造候補を採用して取り下げが起き、その構造候補を却下すると、原因の候補の行の `decision_seq` は却下の `seq` になる）。
  - CSV には `decision_log` が無いので、CSV で原因を引けるのは、原因の候補が決め直されていないときだけである。CSV に原因の `candidate_id` の列を足す案は取らない。列を1つ増やして得られるのは CSV だけで原因を引けることで、そこまでの要求はいま無い。
- **`orphaned_kind` を足した理由**。`orphaned` は「対象の要素が見つからなかった、または操作が当たらなかった」で一律に立つが、そのうち修正が本当に最終HTMLから落ちているのは一部だけである（3.7 の S2、3.13 の S3 からの申し送り）。証跡を読む人が区別できるように、分類を列に出す。分類は `orphanedKindOf()` の1つにまとめ、テスト用に `goal2Engine.decisionLog.orphanedKind` から呼べる。
  - `no-op`: 候補が HTML を元から変えない（`patch_mode` が `"none"`、`op` が無い、または変換後HTMLが変換前と同じ）。修正は失われていない。
  - `target-replaced`: 決定の `node_id` が派生ID（`nX.s{seq}.{k}`、入れ子では `.s` が複数並ぶ）で、その中のいずれかの `seq` の決定がいま効いていない（その候補の最新の決定でない、または採用・編集でない）。構造候補を決め直したために対象が作り直されたもので、修正は失われていない。同じ問題が残っていれば、新しい対象への候補として出直す（採用から却下への決め直しで出直すことを回帰テストで確かめた）。構造候補を編集に決め直したとき（編集の初期値に修正が入っている）や、決め直した先の構造で問題そのものが無くなったときは出直さない。参照先の決定が効いているのにそれ自体が `orphaned` のときは、派生IDの要素がそもそも作られていないので `lost` にする。参照先の `seq` がログに無いときも、決め直したとは言い切れないので `lost` にする（作業者に気付かせる側に倒す）。
  - `lost`: 上のどちらでもない。修正が最終HTMLに入っていない。
  - 画面の詳細欄の説明は、`no-op` が「この候補はHTMLを変えない候補なので、対象が見つからなくても失われた修正はありません。」、`target-replaced` が「構造候補を決め直したために対象の要素が作り直されました。修正は失われておらず、同じ問題が残っていれば、新しい対象への候補として出直します。」、`lost` が「この決定の修正は対象が見つからずに当たらなかったため、最終HTMLに入っていません。」である。
- **証跡 JSON のトップに `decision_log` を足した**。決定ログの各行から `seq`、`generation`、`candidate_id`、`rule_id`、`node_id`、`status`、`actor`、`decided_at`、`withdrawn_by_seq`、`orphaned` を出す。候補の行は最新の決定しか持たないので、決め直した候補の前の決定や、一括採用のかたまり（同じ `generation`）を後から追えるようにするためである。`op` と `after_html` は大きくなるので入れない。`orphaned` は、その候補の最新の決定で、かつ採用・編集の行だけが値を持ち、それ以外は `null` にする。リプレイは最新の決定にしか印を立て直さないので、決め直しで効かなくなった採用の印は評価されていない値で、`false` と出すと「当たった採用」と読めてしまう。
- GOAL1（`goal2Engine.buildEvidence`）は context に決定ログを渡さないので、`decision_seq`・`withdrawn_by_seq`・`orphaned`・`orphaned_kind`・`decision_log`・`completion.withdrawn` は `null` になる。`generation` だけは候補の値（GOAL1 は再導出しないので 0）。`goal1.js` の CSV（13列）は変えていない。GOAL1 に決定ログを渡すのは S5（3.12）。
...
**S4 画面と証跡**。実装済み（PR #141）。
バッジ、取り下げの表示、証跡の列追加。
検証: 証跡CSVの既存列が変わっていないことをファイル比較で確認。

S4で決めた段階差は 3.10・3.11 に書いた。要点は次のとおり。

- 証跡の列は本文の3列（`generation`・`withdrawn_by_seq`・`orphaned`）に `decision_seq` と `orphaned_kind` を足した5列で、JSON のトップに `decision_log` を足した（3.11）。
- `orphaned` は `no-op`・`target-replaced`・`lost` に分け、画面で「最終HTMLに未反映」を出すのは `lost` だけにした。`lost` が残っていても完了判定は変えない（3.10）。
- GOAL1 の証跡の新しいキーは `generation` を除いて `null`（S5 で決定ログを渡す）。

S4の検証結果。

- 同じ入力（回帰テストの入力6件と佐賀市の実ページ5件）と同じ操作（「確認不要をまとめて採用」→構造候補を1件採用→内容修正を1件採用）で `main` と S4 の証跡CSVを出し、取り下げ以外の行の先頭23列が11件すべてファイル比較で一致した。
- 佐賀市の実ページ51件の GOAL1 経路（`analyze()`→`autoAcceptSafe()`→`buildFinalHtml()`）の最終HTMLは、51件すべて `main` と同一（採用件数も同じ）。
- 再導出の時間は、要素数が最も多い sg00761（708要素）で決定1件あたり平均71.7ミリ秒・最大99.2ミリ秒（`main` は平均67.6ミリ秒・最大96.6ミリ秒）。S4 はリプレイと候補生成に触らないので、差は測定のゆらぎの範囲である。
