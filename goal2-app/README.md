...
LLM(Gemini)連携は実装済みだが、`GEMINI_API_KEY` 未設定(既定)では一切呼び出されない。実案件HTML・画像を外部LLMへ送信してよいかの最終的なデータポリシー合意は自治体・発注元との間で未確定のため、有効化は運用判断に委ねる。詳細は [LLM (Gemini) 連携](#llm-gemini-連携) と [LLM_DATA_POLICY.md](LLM_DATA_POLICY.md)(Googleのデータ利用規約の調査結果・実案件投入前の最低条件たたき台)を参照。
