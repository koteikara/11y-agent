...
const TASKS = {
  ...
  "image-alt": {
    systemPrompt:
      ...
      "画像内に文字(バナー・ポスター・チラシの見出しや告知文、日付、案内文など)が描かれている場合、その文字も画像の内容なので、可能な範囲でalt_textに含めてください。" +
      "内容を持たない装飾目的のみの画像(罫線・アイコン等)と判断した場合はis_decorativeをtrueにしてください。" +
      "画像の内容(画像内の文字を含む)を、代替テキスト一行=100文字程度に収められるかで分岐してください。" +
      "収められる場合はis_complexをfalseにし、内容(画像内の文字を含む)をそのままalt_textに入れてください(例:小さな告知バナーなら『7月10日は休館日です』のように文字をそのまま入れる)。" +
      "地図・グラフ・図表・チラシ・ポスターなど、内容が多く100文字程度に収まらない場合はis_complexをtrueにし、alt_textには主題に加えて、画像内の主要な文字や主要な被写体(写真に写っている主なもの)を、代替テキスト一行=100文字程度に収まる範囲でできるだけ盛り込んでください" +
      "(分類語一語や短いラベルで済ませず、使える文字数を活かして具体的に。例:「グラフ」ではなく「令和3〜8年度の人口推移を示す折れ線グラフ」、「観光地特集のポスター」ではなく「海に浮かぶ鳥居や五重塔、海鮮丼やラーメン、もみじ饅頭などを紹介する観光地特集のポスター」)。" +
      "そのうえで、100文字にも入りきらない網羅的な詳細(グラフの全数値、チラシ・ポスターの全項目、写っている全ての写真、画像内の全文字)のみcomplex_detailに回してください。「詳細は以下」という接尾辞は付けないでください(後処理で自動的に付与されます)。" +
      "alt_textに含めなかった網羅的な詳細情報(画像に具体的に何が描かれているか、画像内の文字の全文など。複数の要素がある場合は列挙)は、is_complexがtrueの場合のみcomplex_detailに記述してください。" +
