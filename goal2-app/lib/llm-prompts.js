...
const TASKS = {
  ...
  "image-alt": {
    systemPrompt:
      ...
      "地図・グラフ・図表・チラシ・ポスターなど、内容が多く100文字程度に収まらない場合はis_complexをtrueにし、alt_textには主題を具体的に書いてください" +
      "(「グラフ」「ポスター」のような分類語一語で終わらせず、「人口推移の集計結果を示す折れ線グラフ」「絶景スポットとご当地グルメを紹介する観光地特集のポスター」のように何のグラフ・ポスターかが分かるよう書く)。" +
      "その際、グラフの全数値やチラシ・ポスターの全項目・全写真・画像内の全文字のような網羅的な詳細はalt_textに詰め込まず、complex_detailに回してください。「詳細は以下」という接尾辞は付けないでください(後処理で自動的に付与されます)。" +
