...
> **確認済み**: 2026-07-08、Windows実機でビルド→`goal2-app.exe`の起動→画面表示(KBルール読み込み含む)まで一通り成功を確認した。それでも環境によって問題が起きる場合は、下記「トラブルシューティング」を参照する。KBルール件数はナレッジベースの更新に伴い変わる(2026-07-10時点で58件)。
