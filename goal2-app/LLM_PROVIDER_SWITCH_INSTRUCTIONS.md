...
- 次の2つは、やり直さずに受け皿に回す（L1 の実装で足した。PR #144）。
  - 429（さくらの `llm_rate_limited`、Gemini の 429）。すぐにやり直しても通りにくいためである。
  - 通信の失敗とタイムアウト（`llm_request_failed`）。さくらに SLA が無いためで、やり直すと待ち時間が倍になる。
...
実装済み（PR #144、PR #143 のブランチを基点）。
設計との差異と、設計に書かれていなかった点の扱いは次のとおり。

- `callGemini()` は無くし、中身を gemini アダプター（`requestGemini()`）にした。
- 通信の失敗とタイムアウト（`llm_request_failed`）は、やり直さずに受け皿へ回す。3.7 は 3.4 の 1〜4 だけを受け皿の対象にしていたが、さくらに SLA が無いので足した。429 もやり直さずに受け皿へ回す。レビューを受けて 3.7 にも書き足した。
...
- さくらに送るスキーマは、すべての項目を `required` にし、任意の `string` の `""` は取り出したあとに捨てる（3.3）。最初の実装は元の `required` だけを写しており、L2 で任意の項目がほとんど返らなかったため、あとから直した（PR #149）。
...
- 1回の呼び出しの上限を、本番の 45 秒ではなく 180 秒にした。見出しの見直しの目安（90 秒）を測るためである。
- Gemini 2.5 Flash をもう一度流し（A2）、同じ設定どうしの一致率を揺れの基準として出した。
- 分類の一致率の合否は、真偽や分類の項目で数えた。画面が使わないリンクの `confidence` と、自由度の高い見出しの見直しの `block_id` の集合は、参考として別に出した。
- 画像の候補には、`preview/gemma-4-31B-it` も流した（D）。単価は gemma-4 の公式の料金（入力 24 円、出力 96 円）で計算した。
...
- ~~L0 の本番の設定（案 A、B、C）~~ → 2026-09-25 に案 A で決定し、本番に適用した（4章 L0 の「本番への適用」）。
